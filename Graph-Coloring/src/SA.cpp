...
SA::SA(double initial_temperature, unsigned int n_nodes, unsigned int n_edges,
    ...
    for (unsigned int i = 0; i < n_nodes; i++) chromosome.push_back(1);

    Individual new_individual(chromosome);
    this->best_solution = new_individual;
    this->current_solution = new_individual;
};
SA::SA(double initial_temperature, int coolMode, int alpha,
...
Individual SA::GetBestState() { return this->best_solution; };
...
int SA::findFirstCollision() {
    ...
    if (current_solution.getFitness() < n_edges) {
        for (unsigned int i = 0; i < n_nodes && !collision; i++) {
            for (unsigned int j = 0; j < main_graph[i].size() && !collision;
                 j++) {
                pos = main_graph[i][j];
...
void SA::NextNeighbor() {
    ...
    if (current_solution.at(node) == color) color = (color + 1) % n_nodes;
...
void SA::MainLoop(unsigned int max_iterations, double min_temp,
                  unsigned int& total_iterations, unsigned int min_colors) {
    int correct = 0;
    ...
    for (unsigned int i = 0; i < max_iterations && !correct_color; i++) {
        ...
        if (current_solution.getNumOfColours() == min_colors) correct++;
        if (correct == 10) {
            correct_color = true;
            total_iterations = i;
        }
...
unsigned int SA::CalculateFitness(Individual& ind) {
    ...
    if (ind.getFitness() != 0) {
        ...
        if (fit == n_edges) fit += (n_nodes - ind.getNumOfColours());
